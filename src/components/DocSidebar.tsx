    ...
    <aside className="doc-sidebar">
      ...
      <div className="doc-sidebar-tabs">
        {NAV_SECTIONS.map((section) => (
          <button
            key={section.key}
            className={`doc-sidebar-tab ${currentSection?.key === section.key ? 'active' : ''}`}
        ...
        <nav className="doc-sidebar-nav" aria-label={currentSection.text}>
          {currentSection.items.map((item) => {
            const isActive =
              ...
              (item.path.endsWith('/') && currentPath === item.path.slice(0, -1))
            return (
              <button
                key={item.path}
              ...
              >
                {isActive && <ChevronRight className="doc-sidebar-chevron" size={12} />}
                <span>{item.text}</span>
              </button>
            )
          })}
