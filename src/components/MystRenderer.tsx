...
function preprocessDirectives(md: string): string {
  return md.replace(
    ...
    (_: string, type: string, title: string, body: string) => {
      const titleHtml = title.trim() ? `<div class="admonition-title">${title.trim()}</div>` : ''
...
export function MystRenderer({ content, onNavigate }: MystRendererProps): React.JSX.Element {
